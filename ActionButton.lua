...
function Action:HasDisenchant()
    ...
    if type(IsSpellKnown) == "function" then
        return IsSpellKnown(NS.DISENCHANT_SPELL_ID)
    end
...
function Action:Refresh()
    ...
    if not NS.IsSupportedClient then
        ...
        button.name:SetText("SafeEnchanter")
    ...
    if not self:HasDisenchant() then
    ...
    button.icon:SetTexture(target.texture or "Interface\\Icons\\INV_Enchant_Disenchant")
    SetStatus(L.ACTION_READY .. " - ilvl " .. tostring(target.itemLevel), 0.70, 1.00, 0.70)
    self:Arm(target)
end
...
function Action:HandleSpellSucceeded(unit, spellID)
    if not state.pending or unit ~= "player" or spellID ~= NS.DISENCHANT_SPELL_ID then
        return
    end
    if not NS.Scanner:IsTargetPresent(state.pending.target) then
...
function Action:HandleInventoryChanged()
    if state.pending and not NS.Scanner:IsTargetPresent(state.pending.target) then
...
function Action:Initialize()
    button = CreateFrame("Button", "SafeEnchanterActionButton", UIParent, "SecureActionButtonTemplate,BackdropTemplate")
    ...
    button:SetBackdropColor(0.04, 0.04, 0.06, 0.95)
    button:SetBackdropBorderColor(0.55, 0.25, 0.85, 1.00)
