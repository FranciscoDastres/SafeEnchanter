...
local defaults = {
    profile = {
        enabled = true,
        qualities = {
            [NS.ITEM_QUALITY_UNCOMMON] = true,
            [NS.ITEM_QUALITY_RARE] = false,
        },
        maxItemLevel = 0,
        protectedItemIDs = {},
        showTooltips = true,
...
function Config:InitializeDatabase()
    Addon.db = LibStub("AceDB-3.0"):New("SafeEnchanterDB", defaults, true)
end
...
function Config:RegisterOptions()
    local options = {
        ...
        args = {
            ...
            uncommon = {
                type = "toggle",
                name = L.CONFIG_UNCOMMON,
                order = 20,
                get = function() return Addon.db.profile.qualities[NS.ITEM_QUALITY_UNCOMMON] end,
                set = function(_, value)
                    Addon.db.profile.qualities[NS.ITEM_QUALITY_UNCOMMON] = value
                    NotifyChanged()
                end,
            },
            rare = {
                type = "toggle",
                name = L.CONFIG_RARE,
                desc = L.CONFIG_RARE_DESC,
                order = 21,
                get = function() return Addon.db.profile.qualities[NS.ITEM_QUALITY_RARE] end,
                set = function(_, value)
                    Addon.db.profile.qualities[NS.ITEM_QUALITY_RARE] = value
                    NotifyChanged()
                end,
            },
            maximum = {
                type = "range",
                name = L.CONFIG_MAX_ILVL,
                desc = L.CONFIG_MAX_ILVL_DESC,
                order = 30,
                min = 0,
                max = 200,
                step = 1,
                get = function() return Addon.db.profile.maxItemLevel or 0 end,
                set = function(_, value)
                    Addon.db.profile.maxItemLevel = value
                    NotifyChanged()
                end,
            },
            tooltips = {
    ...
    LibStub("AceConfig-3.0"):RegisterOptionsTable("SafeEnchanter", options)
    LibStub("AceConfigDialog-3.0"):AddToBlizOptions("SafeEnchanter", L.CONFIG_NAME)
...
function Config:Open()
    LibStub("AceConfigDialog-3.0"):Open("SafeEnchanter")
end
